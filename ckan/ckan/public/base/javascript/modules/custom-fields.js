  ...
  return {
    ...
    initialize: function () {
      if (!jQuery('html').hasClass('ie7')) {
        ...
        var delegated = this.options.fieldSelector + ':last input:text:last';
        this.el.on('change', delegated, this._onChange);
        this.el.on('keyup', delegated, this._onKeyup);
    ...
    resetField: function (field) {
      ...
      if (input.val().match(/^custom-authors-/)) {
        input.each(function() {
          var inputt = $(this);

          if (inputt.attr('id').match(/-key$/)) {
            inputt.val(increment).attr('id', increment).attr('name', increment);
            inputt.data('authorId', inputt.val());
            inputt.val('');
          }
          else {
            inputt.val('').attr('id', increment).attr('name', increment);
          }
        });
      }
      else {
